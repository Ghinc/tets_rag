"""
RAG v10 LOCAL - RAPTOR-lite + Sous-questions (modeles locaux via Ollama).

Variante de rag_v10_raptor_subq.py substituant tous les appels API par un
modele local Qwen2.5-7B expose via Ollama (http://localhost:11434/v1).
Ne pas utiliser pour les runs d'evaluation formels — garder rag_v10_raptor_subq.py.

Pipeline en 3 etapes :
  1. Qwen2.5-7B (Ollama) decompose la question initiale en N sous-questions
  2. RaptorRetriever + Qwen2.5-7B (Ollama) repond a chaque sous-question
  3. Qwen2.5-7B (Ollama) synthetise toutes les reponses et filtre le bruit

Usage:
    ollama serve  # dans un terminal separe
    python rag_v10_raptor_subq_local.py --query "..."
    python rag_v10_raptor_subq_local.py --query "..." --n-subquestions 5
"""
...
# --- Modeles locaux via Ollama ---
# Installer Ollama : https://ollama.ai  puis : ollama pull qwen2.5:7b
# Pour plus de qualite (plus lent, ~8 GB VRAM) : ollama pull qwen2.5:14b
LOCAL_MODEL       = "qwen2.5:7b"
DECOMPOSER_MODEL  = LOCAL_MODEL
ANSWERER_MODEL    = LOCAL_MODEL   # remplace claude-haiku
SYNTHESIZER_MODEL = LOCAL_MODEL
MISTRAL_BASE_URL  = "http://localhost:11434/v1"  # endpoint OpenAI-compatible d'Ollama
...
def _call_mistral(prompt: str, system_prompt: str,
    ...
    # Ollama n'exige pas de cle reelle — on utilise une valeur factice.
    api_key = os.getenv("MISTRAL_API_KEY", "ollama")
...
def _call_claude(prompt: str, system_prompt: str,
                 ...
                 max_retries: int = 5) -> str:
    """Appel local via Ollama (interface OpenAI-compatible) en remplacement de Claude Haiku."""
    return _call_mistral(prompt, system_prompt, model=model,
...
def main():
    parser = argparse.ArgumentParser(
        description=f"RAG v10 LOCAL : RAPTOR-lite + Sous-questions ({LOCAL_MODEL} via Ollama)"
    )
