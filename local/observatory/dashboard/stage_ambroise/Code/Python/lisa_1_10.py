...
Version normalisation 1-10 avec seuils de significativité 1% et 5%
"""
...
def charger_donnees(fichier='data_pour_lisa_1_10.csv'):
...
def calculer_lisa(gdf, w, variable, alpha=0.01):
    """
...
def exporter_pour_web(gdf, variable, alpha_list=[0.01, 0.05]):
    ...
    for alpha in alpha_list:
        suffix = f"_{int(alpha*100)}pct"
        output_file = f"lisa_{variable}_1_10_{int(alpha*100)}pct.json"
...
def main():
    """Fonction principale"""
    print("=" * 80)
    print("ANALYSE LISA - NORMALISATION 1-10 (1% et 5%)")
    print("=" * 80)
    print()

    # Charger les données
    df = charger_donnees('data_pour_lisa_1_10.csv')
    ...
    variables = ['OppChoVec_1_10', 'Score_Opp_1_10', 'Score_Cho_1_10', 'Score_Vec_1_10']

    # Pour chaque variable
    for variable in variables:
        print("\n" + "=" * 80)
        print(f"ANALYSE DE: {variable}")
        print("=" * 80)

        # LISA à 1%
        print("\n--- Seuil de significativité: 1% ---")
        lisa_1pct, gdf, moran_1pct = calculer_lisa(gdf, w, variable, alpha=0.01)

        # LISA à 5%
        print("\n--- Seuil de significativité: 5% ---")
        lisa_5pct, gdf, moran_5pct = calculer_lisa(gdf, w, variable, alpha=0.05)

        # Exporter pour le web
        exporter_pour_web(gdf, variable, alpha_list=[0.01, 0.05])
    ...
    # Sauvegarder en CSV
    output_csv = 'lisa_resultats_1_10_complet.csv'
    ...
    if web_dir.exists():
        for variable in variables:
            for alpha in [1, 5]:
                src = f"lisa_{variable}_1_10_{alpha}pct.json"
                if Path(src).exists():
