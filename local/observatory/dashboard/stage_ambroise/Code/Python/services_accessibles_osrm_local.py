...
VERSION OSRM LOCAL - Utilise un serveur OSRM local pour des calculs ultra-rapides

PREREQUIS:
1. Docker installé sur votre machine
2. Serveur OSRM local en cours d'exécution sur localhost:5000

POUR LANCER LE SERVEUR OSRM LOCAL:
  Voir le fichier README_OSRM_LOCAL.md pour les instructions complètes
...
# API OSRM LOCAL - Serveur local au lieu de l'API publique
...
def verifier_osrm_local() -> bool:
    """
    Vérifie que le serveur OSRM local est accessible

    Returns:
        True si le serveur répond, False sinon
    """
    try:
        # Test simple avec deux points en Corse (Ajaccio et Bastia)
        test_url = "http://localhost:5000/table/v1/driving/8.7379,41.9270;9.4496,42.7027?sources=0&destinations=1"
        response = requests.get(test_url, timeout=5)

        if response.status_code == 200:
            data = response.json()
            if data.get('code') == 'Ok':
                print("[OK] Serveur OSRM local accessible et fonctionnel")
                return True

        print("[ERREUR] Serveur OSRM local ne repond pas correctement")
        return False

    except requests.exceptions.ConnectionError:
        print("[ERREUR] Impossible de se connecter au serveur OSRM local sur localhost:5000")
        print("\nPour lancer le serveur OSRM local, voir README_OSRM_LOCAL.md")
        return False
    except Exception as e:
        print(f"[ERREUR] Erreur lors de la verification du serveur OSRM: {e}")
        return False
...
def calculer_matrice_distances_osrm(coords_sources: List[Tuple[float, float]],
    ...
    """
    Calcule la matrice des temps de trajet entre sources et destinations via OSRM LOCAL

    VERSION OPTIMISEE pour serveur local (pas de rate limiting)
    ...
    # BATCH SIZE AUGMENTE car pas de rate limiting avec serveur local
    batch_size = 100  # vs 25 pour API publique

    total_batches = ((n_sources + batch_size - 1) // batch_size) * ((n_dest + batch_size - 1) // batch_size)
    current_batch = 0
    ...
    for i in range(0, n_sources, batch_size):
        ...
        for j in range(0, n_dest, batch_size):
            ...
            try:
                ...
                # PAS DE PAUSE avec serveur local (vs 1.2s pour API publique)
                # time.sleep(0.01)  # Juste une micro-pause symbolique

                current_batch += 1

                # Afficher la progression plus fréquemment
                if current_batch % 10 == 0 or current_batch == total_batches:
                    progress = (current_batch / total_batches) * 100
                    print(f"    Progression: {progress:.1f}% ({current_batch}/{total_batches} batches)")
...
def main():
    ...
    print("VERSION OSRM LOCAL (ULTRA-RAPIDE)")
    print("="*80)

    # Vérifier que le serveur OSRM local est accessible
    print("\nVérification du serveur OSRM local...")
    if not verifier_osrm_local():
        print("\n" + "="*80)
        print("ERREUR: Serveur OSRM local non accessible")
        print("="*80)
        print("\nPour lancer le serveur OSRM local:")
        print("1. Installer Docker Desktop")
        print("2. Suivre les instructions dans README_OSRM_LOCAL.md")
        print("\nOu utilisez services_accessibles_20min.py (API publique)")
        return
    ...
    print(f"\nTemps de calcul: {elapsed_time/60:.1f} minutes ({elapsed_time:.0f} secondes)")
    ...
    # CSV avec suffixe _local pour éviter d'écraser les résultats API publique
    output_csv = 'services_accessibles_20min_local.csv'
    df_resultats.to_csv(output_csv, index=False, encoding='utf-8')
    print(f"\n[OK] Fichier CSV: {output_csv}")

    # Excel
    output_excel = 'services_accessibles_20min_local.xlsx'
    ...
    # JSON
    output_json = 'services_accessibles_20min_local.json'
