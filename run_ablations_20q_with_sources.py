"""Run 4 ablation configs on 20 annotated questions — saves sources content."""
...
print(f"20 questions chargees. Lancement des {len(VERSIONS)} configs avec sources...", flush=True)
    ...
    for i, q in enumerate(questions, 1):
        ...
        try:
            ...
            if r_resp.status_code == 200:
                data = r_resp.json()
                # Sauvegarder les sources avec contenu tronqué à 800 chars
                raw_sources = data.get("sources", [])
                sources_saved = []
                for s in raw_sources:
                    sources_saved.append({
                        "content": (s.get("content") or s.get("extrait") or "")[:800],
                ...
                entry = {
                    ...
                    "status": "error", "error": r_resp.text[:200], "sources": [],
                }
                print(f"  [{i:2}/20] R{q['excel_row']} ERREUR {r_resp.status_code}: {r_resp.text[:100]}", flush=True)
        except Exception as e:
            entry = {"excel_row": q["excel_row"], "question": question,
                     "status": "exception", "error": str(e), "sources": []}
            print(f"  [{i:2}/20] R{q['excel_row']} EXCEPTION: {e}", flush=True)
        results[ver].append(entry)

ts = datetime.now().strftime("%Y%m%d_%H%M%S")
out_path = f"{OUT_DIR}/ablations_20q_with_sources_{ts}.json"
